...
if [ -z "`find LA -type f`" ]; then
  ...
  cp ${SYSTEM_PATH}.pdos_atm#24\(* LA/
  cp ${SYSTEM_PATH}.pdos_atm#25\(* LA/
  cp ${SYSTEM_PATH}.pdos_atm#26\(* LA/
fi
...
mv ${SYSTEM_PATH}.pdos_atm#21\(*  ${SYSTEM_PATH}.pdos_atm#24\(* O_states
cd O_states
...
cat H_tot_LA.dat | awk '{print $1, $2, $3*(-1)}' |  sed '1d' > H_tot_LA_shifted.dat
cd ..
