...
TMPDIR=./
###########Options###################
while getopts x:y:b:a:t:p:d:m:g:w:f:n:s:z:r: opt
do
   case "$opt" in
      x) BIN=$OPTARG;;
      y) RESP=$OPTARG;;
    ...
    -b  Number of nbnd (bands)
    -a  Number of npools
    -t  3D FFT task groups
...
cat $KPOINTS >> ${SYSTEM}.unperturbed_scf_Fe.in
  ...
  if [ ! -f ${SYSTEM}.unperturbed_scf_Fe.out ]; then
     $MPI $PW_BIN $POOL_C $NTG_C -inp  ${SYSTEM}.unperturbed_scf_Fe.in >> ${SYSTEM}.unperturbed_scf_Fe.out 2> /dev/null
...
for a in ${ALPHA[@]}; do
 ...
 if [ ! -f ./${PERTURBED_DIRECT}/${SYSTEM}.perturbed_scf_Fe_$a.out ]; then
    $MPI $PW_BIN $POOL_C $NTG_C -inp ./${PERTURBED_DIRECT}/${SYSTEM}.perturbed_scf_Fe_$a.in  >> ./${PERTURBED_DIRECT}/${SYSTEM}.perturbed_scf_Fe_$a.out 2> /dev/null
 fi # End checking .out exit
done
...
cat $KPOINTS >> ${SYSTEM}.unperturbed_scf_O.in
  ...
  if [ ! -f ${SYSTEM}.unperturbed_scf_O.out ]; then
     $MPI $PW_BIN $POOL_C $NTG_C -inp  ${SYSTEM}.unperturbed_scf_O.in  >> ${SYSTEM}.unperturbed_scf_O.out 2> /dev/null
  fi # End checking .out exit
...
for a in ${ALPHA[@]}; do
 ...
 if [ ! -f ./${PERTURBED_DIRECT}/${SYSTEM}.perturbed_scf_O_$a.out ]; then
    $MPI $PW_BIN $POOL_C $NTG_C -inp  ./${PERTURBED_DIRECT}/${SYSTEM}.perturbed_scf_O_$a.in  >> ./${PERTURBED_DIRECT}/${SYSTEM}.perturbed_scf_O_$a.out 2> /dev/null
 fi # End checking .out exit

# generate dnda files for responsematrix 
#For Fe
