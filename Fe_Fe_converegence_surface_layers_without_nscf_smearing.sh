...
# once is known the wavefunction cutoff (No scf for PDOS and increasing
# value)
# 
#
####################################################################
...
if [[ -z $PO ]]; then    # if PO option present skip this part
 ...
 #  Looop for the different number of layers
  ...
  while test 1 -eq `echo " $N < 19" | bc -l`; 
     do
...
cat $KPOINTS >> ${SYSTEM}_l$N.in
     ...
     if [ -n "$CORRD" ]; then # relaxed coordinates Enters when string is non-zero
       ...
       grep -A 200 "$LASTSTEP"  ${SYSTEM}_l$N.out > TEMP'_fm_' # We have to change the number 150 if we have more than 30 atoms
...
cat $KPOINTS >>  ${SYSTEM}_l${N}_scf.in
   fi #end checking if ${SYSTEM}_l${N}_scf.in already exist
  ...
  if [[ -n $PDOS ]]; then #If one wants to calculate the PDOS
   ...
   if [ ! -f ${SYSTEM}_l${N}_pdos.in ]; then
cat > ${SYSTEM}_l${N}_pdos.in << EOF
 ...
 /
EOF
  fi #end checking if ${SYSTEM}_l${N}_scf.out alredy exist
