...
do
   ...
   Example:time Fe_Fe_converegence_surface_layers.sh -m "mpiexec" -a "6" -w "95" -f "800" -g "PBE" -d "yes" > OUT_FeS_FM_RUN_U_occu_layer110Fe (Running PDOS)
...
echo "pw.x  with n pools executed as: " $MPI $PW_BIN "< input" $POOL_C $NTG_C "> output"
...
#     $MPI $PW_BIN < ${SYSTEM}_l$N.in $POOL_C $NTG_C  > ${SYSTEM}_l$N.out  2> /dev/null
...
#           $MPI $PW_BIN <  ${SYSTEM}_l${N}_scf.in  $POOL_C  >> ${SYSTEM}_l${N}_scf.out  2> /dev/null
...
#        $MPI $PW_BIN < ${SYSTEM}_l${N}_nscf.in $POOL_C  >> ${SYSTEM}_l${N}_nscf.out 2> /dev/null
        echo "Finishing nscf ..."
      fi
...
#      $MPI $PROJWFC_BIN < ${SYSTEM}_l${N}_pdos.in  >> ${SYSTEM}_l${N}_pdos.out 2> /dev/null
